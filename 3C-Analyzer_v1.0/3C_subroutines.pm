...
sub get_time{
	...
	my $duration_m=($monthday2-$monthday1)*24*60 + ($hour2-$hour1)*60 + ($min2-$min1);
...
sub Pre_read_sample_info{
	my ($sample_info_file, $all_raw_files_pointer, $raw_file_format)=@_;
	my @all_raw_files=@$all_raw_files_pointer;
	
	#read sample_info file
	my (%sample_info, %raw_to_sample);
	open my ($IN), "<", $sample_info_file or die;
	while(<$IN>){
		chomp($_);
		...
		my $files_size=0;
		my (@raw_files, @raw_file_names, @fastq_R1_files, @fastq_R2_files, @R1_file_names, @R1_file_names_head, @raw_file_names_head);
		foreach my $raw_name( @raw_names ) {
			...
			foreach my $raw_file(@all_raw_files){
				...
				if ($raw_file_name=~/^$raw_name/i){
					...
					push(@fastq_R2_files, $raw_file) if $raw_file_name=~/_R2_/;
	...
	my %out;
	$out{sample_info_pointer}=\%sample_info;
	$out{raw_to_sample_pointer}=\%raw_to_sample;
	return(\%out);
}
###########################################
sub Pre_initiate_monitor_log{
	my($time_monitor_file, $system_monitor_file, $sample_info_pointer)=@_;
	my %sample_info=%$sample_info_pointer;
	
	my $total_supposed_time=0;
	my $total_files_num=0;
	open my($MON), ">", $time_monitor_file or die; 
	...
	open my($SYS), ">", $system_monitor_file or die;
...
sub refresh_log{
	...
	my %variables;
	#read old data
	system("touch $log_file") unless -f $log_file;
	...
	#export refreshed data
	seek($FH, 0,0); #go to the head of $FH
	foreach my $key( sort {$a cmp $b} (keys %variables ) ){
		print $FH "$key=$variables{$key}\n";
	}
	flock $FH, 8; #unlock file
	close($FH);
...
sub S2_Colocalization_detection{
	...
	my $both_alignment=0;
	my $raw_pairs=0;
	open my($ORI), ">", $variables{result_dir}.'/'.$name.'.original_regions' or die;
	open my($cis_SITE), ">", $variables{result_dir}.'/'.$name.'.cis_ligation' or die;
	open my($trans_SITE), ">", $variables{result_dir}.'/'.$name.'.trans_ligation' or die;
	open my($ONE), ">", $variables{result_dir}.'/'.$name.'.one_alignment' or die;
	open my($NO), ">", $variables{result_dir}.'/'.$name.'.no_alignment' or die;
	open my($MULTI), ">", $variables{result_dir}.'/'.$name.'.multiple_alignment' or die;
	open my($UN), ">", $variables{result_dir}.'/'.$name.'.unknown_mapped_pairs' or die;
...
sub S3_site_detection_one_alignment{
	...
	open my($cis_SITE), ">>", $variables{result_dir}.'/'.$name.'.cis_ligation' or die; # $found=2
	open my($trans_SITE), ">>", $variables{result_dir}.'/'.$name.'.trans_ligation' or die; #$found=4
	open my($un_SITE), ">>", $variables{result_dir}.'/'.$name.'.unmapped_ligation' or die; # $found=8
	open my($multiple_SITE), ">>", $variables{result_dir}.'/'.$name.'.multiple_ligation' or die; # $found=16
	#read one alignment file
	my $fasta_file=$variables{result_dir}.'/'.$name.'_one_alignment.tmp_fasta';
	my $sam_file=$variables{result_dir}.'/'.$name.'_one_alignment.tmp_sam';
	open my($OUT), ">", $fasta_file or die;
	open my($IN), "<", $variables{result_dir}.'/'.$name.'.one_alignment' or die;
...
sub S3_site_detection_no_alignment{
	...
	#read one alignment file
	my %pair;
	open my($IN), "<", $variables{result_dir}.'/'.$name.'.no_alignment' or die;
	my $fasta_file=$variables{result_dir}.'/'.$name.'_no_alignment.tmp_fasta';
	open my($OUT), ">", $fasta_file or die;
	...
	open my($cis_SITE), ">>", $variables{result_dir}.'/'.$name.'.cis_ligation' or die;
	open my($trans_SITE), ">>", $variables{result_dir}.'/'.$name.'.trans_ligation' or die;
	open my($un_SITE), ">>", $variables{result_dir}.'/'.$name.'.unmapped_ligation' or die;
	open my($multiple_SITE), ">>", $variables{result_dir}.'/'.$name.'.multiple_ligation' or die;
	my $sam_file=$variables{result_dir}.'/'.$name.'_no_alignment.tmp_sam';
...
sub S3_site_detection_multiple_alignment{
	...
	my $multiple_ligation=0;
	open my($multiple_SITE), ">", $variables{result_dir}.'/'.$name.'.multiple_ligation' or die;
	open my($IN), "<", $variables{result_dir}.'/'.$name.'.multiple_alignment' or die;
	while(my $line=<$IN>){#2
		chomp($line);
...
#####################################
sub S5_Co_localization_counting{
...
sub S6_Tscore_calculation{
	...
	if($variables{'3C_Tscore'} eq 'yes'){#2
...
sub C4_colocalization{
	...
	my $query_reads=0;
	my $original_regions=0;
	my $fasta_file=$variables{result_dir}.'/'.$sample_name.'.fa';
	open my ($OUT), ">", $fasta_file or die;
	...
	my $sam_file=$variables{result_dir}.'/'.$sample_name.".sam";
	...
	my $unmapped_ligation=0;
	my $multiple_ligation=0;
	open my($cis_SITE), ">>", $variables{result_dir}.'/'.$sample_name.'.cis_ligation' or die; # $found=2
	open my($trans_SITE), ">>", $variables{result_dir}.'/'.$sample_name.'.trans_ligation' or die; #$found=4
	open my($un_SITE), ">>", $variables{result_dir}.'/'.$sample_name.'.unmapped_ligation' or die; # $found=8
	open my($multiple_SITE), ">>", $variables{result_dir}.'/'.$sample_name.'.multiple_ligation' or die; # $found=16
...
#####################################
sub C4_Co_localization_counting{
	my ($variables_pointer, $export_format_pointer)=@_;
	my %variables=%$variables_pointer;
	my %export_format=%$export_format_pointer;
